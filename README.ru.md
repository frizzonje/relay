...
Ретенция — это обещание, а не уборка мусора: сообщения старше `RETENTION_DAYS` (по умолчанию 14) удаляются раз в час, и файл уходит вместе со своим сообщением. `0` — инсталляция не хранит ничего. Загрузку, которую так никто и не отправил, подметает через сутки. Единственное исключение — закреплённые сообщения, и оно намеренно явное: человек сам сказал «это остаётся». До 50 на канал, закрепляет и открепляет тот, кто модерирует сервер, — открепи, и реплика снова живёт те же четырнадцать дней, что и всё остальное.
