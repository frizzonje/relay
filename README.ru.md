...
- **Личные сообщения и звонки** — приватная переписка с человеком, а не с сервером, с глобальным присутствием на всю инсталляцию (в сети / в разговоре / недавно). Звонок из неё даёт собеседнику входящий с «принять»/«отклонить», который можно проигнорировать и продолжать пользоваться интерфейсом (тост в углу на десктопе, баннер сверху на мобиле) — принятый вызов всегда садит двоих в прямой p2p-mesh, SFU для двоих не бывает. Пропущенный звонок оставляет в переписке пунктирную отметку с кнопкой «перезвонить». Честное ограничение: входящий доходит, только пока открыт веб или запущено приложение — пушей у relay нет, и мобильный веб в фоне звонок не поймает
...
|---|---|---|
| `SITE_PASSWORD` | _(пусто)_ | Пароль входа. Общий для api и web. Пусто → авторизация выключена. Это резерв, а не единственное место: пароль, заданный в [панели владельца](#панель-владельца), главнее, и тогда эта строка ни на что не влияет |
...
| `RETENTION_DAYS` | `14` | Сколько живут сообщения и их вложения. `0` (или `forever`) — хранить без срока, `ephemeral` — не хранить вовсе. Читается один раз, на первом старте с пустой таблицей настроек, — дальше срок живёт в [панели владельца](#панель-владельца) |
...
Почти ничего из того, чем инсталляция живёт изо дня в день, в файлах не лежит. У неё есть **владелец** — одна личность, и только она видит в тулбаре щит. За ним панель, в которой инсталляция настраивается на ходу: без редактора и без перезапуска.

Власть даёт машина, а не пароль. На установленном relay:

```bash
relay owner-link     # печатает одноразовую ссылку, годную сутки
```

Из исходников, без CLI:

```bash
docker compose exec api node dist/owner-link.js
```

Кто открыл ссылку — тот и владелец, а прежний им быть перестаёт. Так возвращают себе инсталляцию, когда ключ потерян, — и поэтому же ссылку никому не передают.

Что внутри:

- **95 параметров** на двенадцати вкладках: доступ, люди, модерация, сообщения, файлы, личные, серверы, голос, приглашения, внешний вид, уведомления, обслуживание. Под каждым полем написано, когда правка подействует. Почти все — сразу: ретенция, лимиты загрузки, ограничитель частоты, политика ЛС, режим обслуживания; остальные — для тех, кто подключится после. Четыре поля живут в `.env` и будут жить там всегда (адреса и ключи TURN и SFU): панель их показывает и говорит, где править.
- **Личности** — все, кто когда-либо заходил, с устройствами и отпечатками. Отозвать устройство или забанить человека на всей инсталляции.
- **Баны** — баны на всю инсталляцию и единственное место, где их снимают. Блокировка по адресу — не вкладка, а параметр: `access.blockedAddresses` принимает адреса и маски и закрывает дверь раньше всех прочих вопросов. Это не стена — новый адрес её обходит, — но смена адреса стоит дороже, чем новая личность, и в этой разнице весь смысл.
- **Обслуживание** — выгрузить и загрузить настройки файлом, прогнать ретенцию руками, не дожидаясь часового таймера, разлогинить все устройства, выпустить новую ссылку владельца.
- **Сводка и журнал** — сколько машина ест процессора, памяти и диска, сколько людей и сообщений; и рядом каждая правка и каждое действие с автором и временем.

Секреты сервер не отдаёт. Пароль инсталляции и ключи TURN/SFU видны только как «задано» или «не задано» — ни в панели, ни в журнале, ни в выгрузке настроек их нет.
