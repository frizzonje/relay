...
То есть `relay update`, `docker compose up` и любой рестарт api стирают историю сообщений, оставляя серверы, каналы и файлы. `relay backup` снимает оба тома **и** лежащие рядом конфиги (`.env` со всеми секретами, compose-файл, конфиг Caddy) — машина, собранная заново из такого тарболла, возвращается той же, чего об одних томах сказать было нельзя. Аккаунтов тоже нет: доступ — один общий `SITE_PASSWORD`, а личность — `@`-тег, который посетитель выбирает после входа.
