...
- **Direct messages and calling** — a private conversation with another person, not with a server, backed by installation-wide presence (online / in a call / away). Calling from it rings the other side with accept/decline they can ignore and keep using the app (a corner toast on desktop, a banner on mobile) — an accepted call always seats the two in a direct P2P mesh, never the SFU. A missed call leaves a dashed mark with one-tap "call back". Honest limit: a call reaches the other person only while relay is open in a browser tab or running as an app — relay has no push notifications, so mobile web in the background won't catch one
