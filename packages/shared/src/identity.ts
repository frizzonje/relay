 ...
 * подписывает и другие вещи (сертификат нового устройства — ниже), и подпись,
 * снятая с одного назначения, не должна годиться для другого. Префикс — вся
 * разница между «доказал, что это его ключ» и «доказал, что когда-то подписал
 * 32 случайных байта».
 */
...
/**
 * Сертификат устройства: то, что подписывает донор, впуская новый ключ в свою
 * личность. Читается как утверждение целиком — «ключ такой-то принадлежит
 * личности такой-то», — и подписан он ключом, который в этой личности уже был.
 *
 * Обе половины обязательны. Без личности подпись годилась бы для любой другой
 * (донор, состоящий в двух, впустил бы устройство не туда, куда собирался);
 * без ключа она не про что. Свой префикс — по той же причине, что и у входа:
 * подпись под нонсом не должна превращаться в пропуск для чужого ключа.
 */
...
/**
 * Код связки — шесть цифр. Их набирают на телефоне и диктуют голосом, поэтому
 * не base64 и не слова: цифра однозначна на слух и есть на любой клавиатуре.
 *
 * Секретом код не является и защищает не личность, а новичка: впустить
 * устройство может только тот, кто УЖЕ вошёл, а подобравший чужой код способен
 * разве что перехватить чужое новое устройство в свою личность. От перебора
 * шести цифр за две минуты жизни кода спасает счётчик на сервере.
 */
...
/** Как код выглядит на экране: «428 913» набирается и диктуется вернее слитного. */
export function formatPairCode(code: string): string {
  return `${code.slice(0, 3)} ${code.slice(3)}`;
}

/**
 * Ссылка в QR. Это именно ссылка, а не голые цифры: снятая системной камерой
 * телефона, она открывает relay сама — без «а теперь найдите в приложении
 * экран сканера». Код живёт во фрагменте, и потому не доезжает ни до сервера,
 * ни в его журналы.
 */
export function pairLink(origin: string, code: string): string {
  return `${origin.replace(/\/+$/, '')}/#pair=${code}`;
}

/**
 * Код из того, что дал человек или сканер: голые цифры, «428 913» или ссылка
 * целиком. `null` — это не код, и дальше идти не с чем.
 */
export function readPairCode(raw: unknown): string | null {
  if (typeof raw !== 'string') return null;
  const inLink = raw.match(new RegExp(`[#?&]pair=(\\d{${PAIR_CODE_DIGITS}})`));
  const digits = (inLink?.[1] ?? raw).replace(/[\s-]/g, '');
