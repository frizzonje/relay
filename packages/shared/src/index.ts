...
/**
 * Версия контракта сигналинга — единственное, что клиент обязан сказать о себе
 * на рукопожатии.
 *
 * Нужна она ради одного ответа: «обнови приложение» вместо белого экрана. До
 * 1.0 версии не было вовсе, и клиент, отставший от сервера, вёл себя как
 * сломанный — молчащие каналы, отбитые входы, вечное «переподключаюсь», — а
 * человек читал это как поломку relay. Теперь сервер такому отказывает во
 * входе и говорит, чего именно не хватает.
 *
 * Поднимается вместе с мажором и только с ним: каждый подъём — это отказ во
 * входе всем, кто не обновился, то есть решение, а не деталь реализации.
 */
...
export interface ChatMessage {
  ...
  system?: boolean;
  /** Реакции на сообщение: эмодзи → те, кто его поставил. */
...
/**
 * Кто поставил реакцию: отпечаток ключа и ник, каким он был в тот момент.
 *
 * Именем реакция подписывалась до 1.0, и это была последняя дверь, открытая
 * самоназванием (audit S1): переименовавшийся в чужой ник снимал чужую реакцию
 * и ставил свою от чужого имени. Ники свободные и не уникальные — сверять по
 * ним нечего.
 *
 * Отпечатка нет у того, кто реагировал до 1.0, и у клиента без ключа.
 */
...
/** Реакции сообщения: эмодзи → те, кто его поставил. Пустые ключи сервер удаляет. */
...
export interface Server {
  ...
  locked?: boolean;
  /**
   * Пароль этого закрытого сервера уже предъявлен ЭТИМ соединением — сам ли
   * человек его ввёл или пропуск уехал в handshake (см. lib/unlock-tokens).
   * Считается на сервере под каждый сокет отдельно, и клиент рисует замок по
   * нему, а не по памяти вкладки: перезагрузка память стирает, а пропуск —
   * нет, и без этого флага замок возвращался бы на уже открытый сервер, требуя
   * пароль, который серверу давно не нужен.
   */
  unlocked?: boolean;
  /**
...
export interface ConfigResponse {
  ...
  version?: string;
  /**
   * Unix-время (секунды), когда перестанет работать выданная пара для TURN.
   * Пары временные и у каждого свои, поэтому конфиг перестаёт быть верным сам
   * по себе — вкладке, открытой со вчера, нужно знать, когда его перечитать.
   * Поля нет вовсе, если TURN статический или его нет совсем.
   */
  iceExpiresAt?: number;
}
