...
// Каталог настроек инсталляции: один список параметров с типами, границами и
// умолчаниями. По нему сервер проверяет запись, а панель владельца сама рисует
// контролы — см. ./settings.
export * from './settings';
// Тип нужен и здесь, по имени: `export *` пробрасывает имена наружу, но в
// область видимости этого файла их не вносит. Импорт типовой (стирается
// сборкой), поэтому встречный `import type` в settings.ts цикла не заводит.
import type { SettingValue } from './settings';

// Машина состояний вызова: чистый переход, по которому владелец живых вызовов
// на сервере и оба экрана — исходящий и входящий — одинаково решают, чем
// кончился дозвон. Считается одним кодом на всех сторонах — см. ./ring.
export {
  missed,
  settled,
  step,
 ...
 *
 * Она не сообщение: её никто не говорил, и рисуется она пунктирной плашкой, а
 * не пузырём (референс, экран 3). Поэтому в ленту она едет структурой, а не
 * фразой: текст строки — язык читающего, а он у сервера не спрашивается
 * никогда, да и «21:04 · 38 с дозвона» из строки обратно не разобрать.
 *
 * Два поля, и оба несут то, чего из самой ленты не видно. `state` — почему
 * звонок пропущен, и исходов у него ровно два: «не ответили» и «не в сети».
 * Остальные четыре отметки не оставляют вовсе (см. `missed` в ./ring), и
 * поэтому третьего значения здесь нет — не «пока нет», а не бывает. `ms` —
 * сколько звонило: `endedAt - startedAt` дозвона, обе точки по часам сервера.
 * Время самой отметки — `ts` реплики, и второй копии времени тут нет
 * намеренно: по `ts` строка стоит в ленте, и разойдись эти два числа, отметка
 * говорила бы одно, а лежала бы в другом месте разговора.
 */
export interface CallMark {
  state: 'no-answer' | 'failed';
  /** Сколько длился дозвон, мс. Ноль — законное значение, отрицательного не бывает. */
  ms: number;
}
...
export interface ChatMessage {
  ...
  system?: boolean;
  /**
   * Эта строка — отметка о пропущенном звонке, а не реплика. Стоит рядом с
   * `system`, а не вместо него: системная она и есть (её никто не писал, её не
   * правят и не удаляют), но рисуется совсем иначе — плашкой с «перезвонить», а
   * не курсивом по центру. Клиент, который поля не знает, покажет её обычной
   * системной строкой и не соврёт: `text` для того и написан словами.
   */
  call?: CallMark;
...
/**
 * С этого начинается имя голосовой комнаты беседы — той, в которую принятый
 * вызов сажает двоих.
 *
 * Приставка не украшение: адрес беседы это ещё и слаг её ленты, а комнаты
 * socket.io у ленты и у эфира общие. Без неё сигналинг звонка уезжал бы
 * каждому, кто просто открыл переписку.
 */
export const CALL_ROOM_PREFIX = 'voice:';

/**
 * Комната беседы по её адресу. Считать её клиент не может (адрес выводится из
 * id личностей, а их в протоколе нет) — имя приезжает готовым в `call-state`
 * с состоянием `accepted`; эта функция лишь называет правило, по которому
 * сервер его составил.
 */
export function callRoomOf(address: string): string {
  return CALL_ROOM_PREFIX + address;
}

/**
 * Комната беседы это или обычный голосовой канал. Проверяется форма целиком, а
 * не одна приставка: «voice:болталка» — законное имя комнаты (тот же довод,
 * что у `isDmSlug`).
 */
...
/**
 * Где человек сейчас — на всю инсталляцию, а не в отдельном канале.
 *
 * `voice-presence` отвечает на другой вопрос — «кто в этой комнате», — и для
 * дозвона он бесполезен: звонят человеку, а не комнате. `recent` — «только что
 * был»: закрыл вкладку, потерял сеть, вышел на минуту; `offline` в снимке не
 * встречается вовсе (нет записи — нет и человека) и приходит только дельтой,
 * которой у собеседника гаснет точка.
 */
export type PresenceState = 'online' | 'in-voice' | 'recent' | 'offline';

export interface PresenceEntry {
  /** Кто: отпечаток ключа — им человек назван всюду, где он адресат. */
  fingerprint: string;
  state: PresenceState;
...
export interface ConfigResponse {
  ...
  iceExpiresAt?: number;
  /**
   * Действующие настройки инсталляции — тот самый снимок, что отдаёт
   * `SettingsService.public()`, без единого секрета (их каталог заменяет
   * признаком «задано»).
   *
   * Второго списка «что можно показывать» нет намеренно: два списка разъезжаются
   * молча и всегда в сторону утечки. Поле необязательное — сервер прошлой версии
   * его не пришлёт, и тогда клиент живёт на умолчаниях каталога, то есть ровно
   * так, как жил до этапа C.
   */
  settings?: SettingsSnapshot;
}

/**
 * Снимок настроек, каким его видит браузер. Ключи — из каталога
 * (`packages/shared/src/settings.ts`), значения уже обезврежены: у секрета
 * вместо строки признак «задано».
 */
...
export interface CallStateRelay {
  ...
  video: boolean;
  /**
   * Голосовая комната беседы — только у `accepted`, и это единственное место,
   * где её адрес называют.
   *
   * Вывести его клиент не может: адрес считается из id двух личностей, а id в
   * протоколе не появляется вовсе (§9.3). У `ringing` комнаты ещё нет — вызов
   * заводит её ровно в миг ответа.
   */
  room?: string;
}

/**
 * Разговор кончился — для тебя.
 *
 * Приходит в двух случаях, и оба для вкладки означают одно: собеседник ушёл из
 * комнаты (разговора двоих на одного не бывает) либо разговор переехал на
 * другое твоё устройство. Сервер к этому мгновению уже вывел сокет из комнаты;
 * событие нужно затем, чтобы вкладка сняла экран звонка и отпустила микрофон —
 * без него она сидела бы в разговоре, которого нет.
 */
export interface CallOverRelay {
  room: string;
}
...
export interface ServerToClientEvents {
  ...
  /**
   * Кто сейчас на связи во всей инсталляции — снимок на подключении. Приходит
   * только тому, кто предъявил личность: гостю по инвайту присутствия нет ни
   * одной стороной.
   */
  presence: (people: PresenceEntry[]) => void;
  /**
   * Изменившиеся записи присутствия — дельтой, а не снимком: картина на большой
   * инсталляции длиннее любого изменения в ней, а меняется она на каждый вход,
   * выход и звонок. Рассылка коалесцируется, поэтому в одном событии приезжает
   * пачка.
   */
  ...
  /**
   * Разговор кончился: собеседник вышел из комнаты беседы — или разговор
   * переехал на другое устройство человека. Из комнаты сервер уже вывел; это
   * событие говорит вкладке снять экран звонка и отпустить микрофон.
   */
  ...
  /**
   * Действующие настройки инсталляции — на подключении и при каждом изменении,
   * всем сокетам одинаково.
   *
   * Снимком целиком, а не правкой одного ключа, по той же причине, что у
   * реестра серверов: вкладка, пережившая обрыв, обязана получить действующее
   * состояние, а не догонять пропущенные патчи. Настройка, доезжающая только
   * после перезагрузки, — половина настройки.
   */
   ...
   * не пустят. Показать экран вместо молчащего приложения обязательно.
   *
   * Тело необязательное: владелец мог объяснить причину словами
   * (`moderation.banNotice`), а мог и не объяснять — и тогда событие приходит
   * таким же пустым, каким приходило всегда.
   */
  banned: (payload?: BannedRelay) => void;
  /**
  ...
  /**
   * В голосе отказали, и вот почему: канал полон, гостей по ссылке больше не
   * пускают, камера или демонстрация экрана выключены владельцем. Летит только
   * отказанному — событиям голоса ответа не положено, а `join` без этого
...
export type ChatRefusal =
  ...
  | 'reactions-off'
  | 'search-off'
  /** Личность моложе тихого часа: читать можно, писать ещё нет. */
  | 'too-new'
  /** Спойлеры выключены: отправить открыто — не то, о чём просили. */
  | 'spoiler-off';
 ...
 *
 * `not-in-call` — комната беседы: разговор двоих, и спрашивающий не один из
 * них (либо разговора уже нет вовсе). Два этих случая нарочно отвечают
 * одинаково: разбери их сервер по-разному, посторонний узнавал бы, что беседа
 * с таким адресом существует.
 */
...
/** Бан — тому, кого забанили. Текст пишет владелец, и он может молчать. */
...
export interface ChatClosedRelay {
  ...
  reason?: 'banned';
  /**
   * Что сказал владелец (`moderation.banNotice`). Приходит только вместе с
   * `reason: 'banned'` и только если тексту есть чем быть: удаление канала
   * объяснять нечем, а пустая настройка — это молчание, а не пустая строка.
   */
  notice?: string;
}
