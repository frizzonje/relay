 ...
 * И убирают за собой: заведённые серверы сносятся после каждого теста —
 * потолок в двадцать серверов на инсталляцию (MAX_SERVERS) полный прогон
 * выбирал бы к третьему разу, причём молча: сервер просто не создавался, а спек
 * падал через двадцать секунд на «где мой канал».
 */
