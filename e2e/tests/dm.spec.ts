...
test('док ЛС раздвигает раскладку, а свёрнутый возвращает её как была', async ({ browser }) => {
  ...
  // середину хода — панель уже 232, колонка ещё 14. Ждём именно конец хода, а
  // не «почти раскрылась»: состав и док меряются по очереди, и растущий док
  // между двумя замерами успевал сдвинуть свой край на пару точек.
  await expect.poll(async () => (await rect('dm-dock')).width, { timeout: 20_000 }).toBe(232);
