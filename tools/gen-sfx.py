...
SOUNDS = {
    # входящий звонок — единственный из набора, что зациклен в плеере
    # (apps/web/lib/sfx.ts, `LOOPING`): звонит, пока вызов жив, а не один раз.
    # Трель в четыре удара (E3 A3 E3 A3), суше и громче остальных сигналов
    # эфира — цикл с паузой в хвосте читается как настойчивый повторяющийся
    # звонок, а не как ещё один peer-join.
    "ring":       mix([ev(E3, 0.15, 0.00, 0.5), ev(A3, 0.15, 0.11, 0.5),
                       ev(E3, 0.15, 0.22, 0.5), ev(A3, 0.15, 0.33, 0.5)],
                      tail=0.75, wet=0.20, predelay=0.006, level=0.72),
    ...
    # отправлено — самый тихий сигнал набора. Это подтверждение своему же
    # действию, а не новость: человек и так знает, что нажал Enter. Ниже
    # "message" и по высоте, и по уровню, срез темнее — звук должен ложиться
    # под палец и не поднимать головы от клавиатуры.
    "send":       mix([ev(D3, 0.70, 0.00, 0.26)], tail=0.80, wet=0.10,
                      predelay=0.006, level=0.15, voice=SOFT, lp=820.0),
    # получено — тот же тембр квинтой выше. Пара "отправил/пришло" так читается
    # как вопрос и ответ, а не как два разных прибора; чуть громче отправки,
    # потому что чужое действие, в отличие от своего, заранее не известно.
    "receive":    mix([ev(A3, 0.70, 0.00, 0.26)], tail=0.85, wet=0.12,
                      predelay=0.006, level=0.20, voice=SOFT, lp=980.0),
}
