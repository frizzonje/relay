...
| `access.blockedAddresses` | list CIDR (до 500) | пусто | now |
...
Проверка счётом (её же делает тест `SETTINGS.length === 95`): access 11, people 6,
...
### Задача 11а: Бан по адресу

**Зачем.** Бан по ключу останавливает происходящее ровно на секунду. Личность в relay
рождается на устройстве без чьего-либо разрешения — это записано в `roles.service.ts` как
осознанная цена, — и забаненный заводит новую быстрее, чем владелец закрывает панель.
Адрес сменить дороже: не невозможно, но дороже, и этой разницы хватает, чтобы «прекрати
сейчас же» стало исполнимым.

**Чем это НЕ является.** Защитой от нагрузки: запрос всё равно доходит до Node, оплатив
TLS. Инструмент против потока — файрвол или Caddy перед приложением, и панель его не
заменяет. И не запретом человеку: VPN и мобильный интернет обходят такой бан за минуту.
Обе оговорки обязаны стоять в подсказке поля, а не только здесь: настройка, от которой
ждут больше, чем она даёт, хуже отсутствующей.

**Файлы:** `packages/shared/src/settings.ts` + `apps/api/src/settings/catalog.ts`
(близнецы, правятся одинаково), `apps/api/src/gateway/perimeter.ts`,
`apps/api/src/gateway/signaling.gateway.ts`, `apps/api/src/http-gate.ts`,
`apps/api/src/auth/auth.ts`, `apps/web/components/providers/SocketProvider.tsx`,
`apps/web/stores/contract.ts`, `apps/web/components/layout/BlockedGate.tsx`, i18n, тесты.

**Решения, принятые заранее:**

1. **Префикс, а не адрес.** Список хранит CIDR: `1.2.3.4`, `10.0.0.0/8`, `2001:db8::/32`.
   Голый IPv6 без префикса — не настройка, а её видимость: жильё раздаётся блоком /64, и
   адрес внутри него меняется сам собой (RFC 4941). Бан одного `/128` перестаёт
   действовать через несколько минут, ничего об этом не сказав, — то есть выглядит
   работающим и не работает. Поэтому голый IPv6 разворачивается в `/64`, и панель
   показывает, во что развернула.
2. **Владельца не блокирует никогда.** Тот же порядок, что у режима обслуживания
   (`perimeter.recognize`), и по той же причине — не из почтения. Единственный путь назад
   на самостоятельной машине — ssh; опечатка в маске иначе стоила бы владельцу доступа к
   собственной панели, а починить это из панели было бы уже нечем.
3. **Обе двери.** Сокет (`perimeter.recognize` → отказ `blocked`) и http (`authGate`).
   Только сокет означал бы, что заблокированный по-прежнему грузит файлы и жжёт диск.
   Порядок на http важен: сперва дешёвая проверка по списку в памяти, и лишь для
   попавшего под маску — выяснение владения; обычный запрос не платит за это ничего.
4. **Отказ отдельный от бана.** За одним адресом сидит подъезд, институт, оператор:
   попавший под маску мог не делать ничего. «Ваш адрес закрыт» и «вы забанены» — разные
   сообщения, потому что делать по ним надо разное, и второе вдобавок обвиняет невиновного.
5. **Клиенту не уезжает** (`client` не ставим): список того, с кем воюет владелец, — не
   дело чужого браузера, и уж точно не гостевого.
6. **Разбор живёт внутри близнецов.** `validateSetting` зовёт и браузер, и сервер, а
   `apps/api` намеренно не ввозит `@relay/shared`; значит, парсер CIDR — обычная функция
   в обоих файлах, и его одинаковость держит тот же тест на близнецов.

- [x] **Шаг 1: Тест разбора и каталога.** Принимаются `1.2.3.4`, `10.0.0.0/8`,
      `2001:db8::/32`, `::1`; отвергаются с `bad-item` — `1.2.3.4/33`, `10.0.0.0/-1`,
      `2001:db8::/129`, `не адрес`, пустая строка, `1.2.3.4 ` с пробелом. Голый
      `2001:db8::1` сохраняется как `2001:db8::/64`. Совпадение: `10.1.2.3` под
      `10.0.0.0/8`, `10.1.2.3` НЕ под `10.0.0.0/16`, IPv4 не совпадает с IPv6-маской.
      `SETTINGS.length === 95`, группа access — 11.
- [x] **Шаг 2: Тест двери.** Заблокированный не входит ни сокетом (отказ `blocked`, не
      `banned`), ни по http (401/403). Владелец с того же адреса входит обеими дверями.
      Незаблокированный не вызывает проверку владения вовсе (счётчик обращений к
      `OwnerService` равен нулю). Пустой список никого не блокирует.
- [x] **Шаг 3: упасть → реализовать → зелёные.** 974 теста api, покрытие
      97.49 / 92.21 / 98.13 / 97.49. Разбор CIDR лежит в близнецовой области каталога,
      дверь по адресу передаётся http-гейту подстановкой (`useAddressDoor`) из
      `afterInit` гейтвея — контейнера у express-миддлвары нет.
- [x] **Шаг 4: Экран отказа.** `BlockedGate` — сосед `MaintenanceGate`: сокета у
      отвергнутого нет, поэтому всё вокруг декорация. Текст свой, не банный, и без кнопки
      «повторить».
- [x] **Шаг 5: Живой стек.** `docker compose -p relay up -d --build` и подтверждение, что
      api поднялся: тесты собирают сервисы руками и старта контейнера не проверяют
      (см. `app.module.test.ts`).
- [x] **Шаг 6: Коммит** — `feat(admin): blocking by address`
...
- [x] **Шаг 1: Тест** — плитки сводки (cpu, память, диск, аптайм, люди, сообщения, версия);
      ...
      копируется.
- [x] **Шаг 2–4: упасть → реализовать → зелёные.** 673 теста веба, 55 из них — панель.
      Третья вкладка названа `upkeep`: `maintenance` занято ГРУППОЙ параметров, и это второе
      такое столкновение после `people` → `identities`.
      Машинные цифры (процессор, память, диск, аптайм) берутся своим запросом
      `GET /api/metrics` — в сводке панели их нет вовсе, а второе место для тех же чисел
      однажды разошлось бы с первым.
      Ключ владельца живёт в кадре экрана и не кладётся в стор: он существует в читаемом
      виде один раз, а стор переживает переключение вкладок (тест ищет его в сериализованном
      сторе и требует, чтобы там его не было).
- [x] **Шаг 5: Коммит** — `feat(admin): overview, audit log and maintenance actions`
...
- [x] **Шаг 1: e2e** — `e2e/tests/admin.spec.ts`, четыре теста: ретенция через панель и
      ответ `/api/config`; отсутствие щита у не-владельца (и отсутствие владельческих
      параметров в публичном снимке); обслуживание с обеих сторон сразу; вкладки на 375.
      Владелец на весь файл один: ссылка сгорает с первого раза, поэтому личность
      заводится однажды, а тесты идут `mode: 'serial'`. Ретенция ставится 1234 дня, а не
      1 день, как было в наброске: спеки гоняются по общему стенду, и день хранения
      подмёл бы соседям переписку прямо посреди их проверок.
- [x] **Шаг 2: Прогнать e2e — упасть → починить → зелёные.** Красный был дважды и оба раза
      по делу: панель помнит вкладку, на которой её закрыли (ждать сводку нельзя), а
      группы приезжают каталогом — панель, пойманная до его приезда, показывает пять
      вкладок из семнадцати.
- [x] **Шаг 3: Вкладки переносятся по строкам на 375** — не две строки, а четыре:
      семнадцать вкладок в две на такой ширине не встают вовсе. Горизонтальная лента
      убрана целиком (`flex-wrap` вместо `overflow-x-auto`), десктопная колонка не
      тронута. Красный тест был честный: шестая вкладка («Файлы») уже не попадала в кадр.
      Заодно нашлось и починено настоящее: в русской локали ДВЕ вкладки звались
      «Обслуживание» (группа `maintenance` и вкладка `upkeep`) — группа стала «Режим
      обслуживания», а неповторимость имён вкладок теперь стережёт тест словарей.
- [x] **Шаг 4: Полный гейт**
...
- [x] **Шаг 5: Коммит** — `feat(admin): mobile layout, docs and the e2e run`
