...
- [ ] **Шаг 1: Тест разбора и каталога.** Принимаются `1.2.3.4`, `10.0.0.0/8`,
...
- [ ] **Шаг 2: Тест двери.** Заблокированный не входит ни сокетом (отказ `blocked`, не
...
- [ ] **Шаг 3: упасть → реализовать → зелёные.**
- [ ] **Шаг 4: Экран отказа.** `BlockedGate` — сосед `MaintenanceGate`: сокета у
      отвергнутого нет, поэтому всё вокруг декорация. Текст свой, не банный, и без кнопки
      «повторить».
- [ ] **Шаг 5: Живой стек.** `docker compose -p relay up -d --build` и подтверждение, что
      api поднялся: тесты собирают сервисы руками и старта контейнера не проверяют
      (см. `app.module.test.ts`).
- [ ] **Шаг 6: Коммит** — `feat(admin): blocking by address`
