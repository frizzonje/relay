...
Отдельный случай — сервер без домена, по голому публичному IP. Сертификаты на IP-адреса Let's Encrypt выдаёт только в профиле `shortlived` (160 часов), и без явного указания профиля Caddy для IP в ACME не пойдёт вовсе. Профиль задаётся файлом `infra/tls-mode.caddy`, который подключается из `Caddyfile` по шаблону `import tls-*.caddy`: в репозитории он пустой (одни комментарии), а непустым его делает `install.sh`, когда пользователь выбрал этот режим. Вторым в списке стоит `issuer internal` — если ACME не прошёл, сайт поднимется с самоподписанным сертификатом вместо отказа.
