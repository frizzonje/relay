 ...
 * «подождите», и клиент обязан различать их без чтения тела. Полный список
 * устройств — 409: код верен, подпись верна, но состояние личности таково, что
 * впустить некуда, и ждать тут бесполезно — надо отозвать лишнее.
 */
function status(reason: PairFailure): number {
  if (reason === 'too-many') return 429;
  return reason === 'too-many-devices' ? 409 : 400;
}
