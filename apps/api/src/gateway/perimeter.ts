...
export class Perimeter {
   ...
   *
   * Из «сирот» выпадает ровно одна комната, и её этот метод не стережёт:
   * комната беседы (`voice:dm-…`). За ней тоже нет канала реестра, но замок у
   * неё свой и не про пароль — список двоих, которым её открыл принятый вызов
   * (`VoiceSessions.mayEnterCallRoom`, спрашивается в `join` рядом с этим).
   */
