 ...
 * 3. **Картина считается целиком из живых сокетов — но ей нужен повод.**
 *    `touch` и `drop` не записывают состояние, они лишь говорят «посмотри
 *    заново», и посмотрев, класс всегда видит правду (см. `picture`). Чего он
 *    НЕ умеет — заметить перемену сам: не позови его никто, разосланная
 *    картина останется вчерашней сколь угодно долго.
 *
 *    Поэтому поводов ровно два, и оба обязаны быть на месте: подключение и
 *    отключение сокета (их знает гейтвей) и любая перемена состава эфира — её
 *    сообщает сама `VoiceSessions` через `onVoiceChanged`. Второй повод отдан
 *    ей, а не обработчикам `join`/`leave`, потому что из эфира вынимают не
 *    только они: ещё бан с сервера (`Moderation.evictFrom`) и истёкший грейс
 *    обрыва — оба через `VoiceSessions.leave`. Перечисляй эти пути гейтвей
 *    поимённо, забытый означал бы человека, вечно «звонящего» в глазах
 *    остальных, — и первым это прочла бы механика дозвона как «занято».
 *
