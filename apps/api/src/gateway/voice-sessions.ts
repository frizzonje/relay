 ...
 * 5. **Комната не переживает разговор.** Конец беседы говорится В КОМНАТУ, а не
 *    по сокетам поимённо, и это единственный способ достучаться до того, у кого
 *    в этот миг моргнула сеть: отключившийся сокет адаптер снимает с комнат сам
 *    (`Socket._onclose` → `_cleanup` → `leaveAll`), адресовать его уже нечем —
 *    зато рассылка в комнату ложится в буфер восстановления и переигрывается
 *    по комнатам сохранённой сессии (socket.io-adapter, `restoreSession`).
 *    Страховка на случай, когда переигрывать нечего (сессия истекла, offset
 *    потерян), — `resumeCall`: вернувшаяся сессия сама спрашивает, её ли ещё
 *    это место (см. `endCall`, `resumeCall`).
 */
export class VoiceSessions {
  ...
  occupants(room: string, client: AppSocket, only?: 'guests'): number {
    ...
    for (const id of this.server.sockets.adapter.rooms.get(room) ?? []) {
      ...
      // Живых спрашиваем по карте сокетов, а не по составу комнаты. Окна, в
      // котором id висел бы в комнате без сокета, у нашего адаптера нет
      // (`leaveAll` идёт раньше `nsp._remove`), но проверка стоит копейки и
      // переживёт смену адаптера — как и такая же в `peersIn`.
      const sock = this.server.sockets.sockets.get(id);
      if (!sock) continue;
  ...
  peersIn(room: string): {
    ...
    // Только реально подключённые. Отвалившегося пира адаптер снимает с комнат
    // сам, так что список и без фильтра был бы верен, — но offer слать некому и
    // в том случае, если адаптер (кластерный, чужой) окажется другим.
  ...
  openCallRoom(room: string, members: [string, string]): string {
    // Повторное открытие того же адреса — не выдумка: те же двое звонят снова,
    // и при выключенном `calls.busyWhenInVoice` — прямо посреди своего живого
    // разговора. Прошлый будильник тогда снимаем (сработав, он кончил бы новый
    // разговор по счёту старого), а вот МЕСТА обязаны пережить переоткрытие:
    // обнули мы их — сидящие остались бы в комнате без мест, `freeSeat` отвечал
    // бы «ушёл не участник» обоим, и разговор стало бы нечем кончить вовсе.
    const open = this.talks.get(room);
    if (open) clearTimeout(open.timer);
    const seats = open?.seats ?? new Map<string, string>();
    // Сторож спрашивает «а сел ли второй», и спрашивает у мест, а не у живых
    // сокетов: моргнувший сокет исчезает из карты немедленно (адаптер снимает
    // его сам), и счёт живых кончал бы разговор ровно тем обрывом, который
    // переживают и окно восстановления, и грейс. Место же освобождает только
    // настоящий уход (`freeSeat`), поэтому «мест меньше двух» истинно ровно
    // тогда, когда второй не сел.
    const timer = setTimeout(() => {
      const still = this.talks.get(room);
      if (still && still.seats.size < 2) this.endCall(room);
    }, VoiceSessions.CALL_ROOM_SEAT_MS);
    timer.unref?.();
    this.talks.set(room, { members, seats, timer });
    return room;
  }
  ...
  private takeSeat(client: AppSocket, room: string): void {
    ...
    // Место переживает свой сокет: прошлое устройство могло отвалиться, и
    // тогда выселять некого — из комнаты его снял адаптер, а место уже занято
    // строкой выше. Вернётся сессия — `resumeCall` увидит, что место не её.
   ...
   *
   * Говорим В КОМНАТУ, а не по сокетам поимённо, и это не стилистика.
   * Участника, у которого моргнула сеть, в комнате уже нет — адаптер снял его
   * вместе с сокетом, — то есть обойти комнату поимённо значит не сказать ему
   * ничего. Рассылка же в комнату ложится в буфер восстановления и
   * переигрывается по комнатам сохранённой сессии, то есть доедет вместе с ней
   * (а не доедет — его встретит `resumeCall`).
   *
