...
export class SettingsService implements OnModuleInit {
  ...
  /**
   * Что уезжает в браузер — любой, включая гостя по ссылке приглашения.
   *
   * `public()` показывает владельцу ВСЁ, кроме секретов, и для панели это
   * верно. Но разослать то же самое каждой вкладке значит раздать заодно
   * список стоп-слов тем, против кого он заведён, и пороги блокировки тому,
   * кто подбирает пароль. Секретом эти значения не назвать — владелец их
   * видит открытым текстом; а вот чужому браузеру они не нужны ни для чего.
   *
   * Отбор — по пометке каталога (`client: true`), а не по второму списку,
   * собранному здесь: список, живущий отдельно от параметров, разъезжается с
   * ними молча. Пометка стоит ровно у тех, кем клиент ПОЛЬЗУЕТСЯ, и что он
   * ими пользуется, держит тест.
   */
  snapshot(): Record<string, SettingValue> {
    const all = this.public();
    const out: Record<string, SettingValue> = {};
    for (const spec of SETTINGS) if (spec.client) out[spec.key] = all[spec.key];
    return out;
  }
