 ...
 * `reactions` — `{ эмодзи: [{ отпечаток, ник }] }` в jsonb: набор эмодзи
 * закрыт, а отпечаток адресует личность так же, как в `mentions`, — отдельная
 * таблица дала бы внешний ключ ценой джойна на каждую страницу ленты.
 */
@Entity('messages')
...
export class MessageRow {
  ...
  /**
   * Отметка о пропущенном звонке: `{ state, ms }` — почему пропущен и сколько
   * звонило. Пусто у всего остального, включая обычные системные строки.
   *
   * Своей колонкой, а не фразой в `text`, потому что читать её обратно
   * приходится машине: «21:04 · 38 с дозвона» клиент собирает сам, на языке
   * читающего, — а из готовой строки эти числа не разобрать. Текст при этом
   * остаётся написанным словами: клиент, не знающий поля, покажет обычную
   * системную строку и не соврёт.
   *
   * Дефолта нет намеренно (в отличие от `reactions`/`mentions`): пустая отметка
   * — это не «отметка ни о чём», а её отсутствие, и `null` говорит это прямо.
   */
  @Column({ type: 'jsonb', nullable: true })
  call!: { state: 'no-answer' | 'failed'; ms: number } | null;
  ...
  @Column({ type: 'jsonb', default: {} })
  reactions!: Record<string, { fingerprint?: string; nick: string }[]>;
...
/**
 * Настройка инсталляции. Строка на ключ, значение — jsonb.
 *
 * Строкой на ключ, а не документом на инсталляцию: у каждого параметра свой
 * автор и своё время правки, и это ровно то, что спрашивают у журнала («кто
 * закрыл регистрацию»). Документ хранил бы одно время на все девяносто восемь.
 *
 * `value` — `unknown`: что именно лежит под ключом, знает каталог параметров, а
 * не таблица. Обещать здесь тип значило бы обещать его дважды и разъехаться.
 *
 * Внешнего ключа на личность нет — по той же причине, что у соседей слоя 3:
 * ушедший владелец не должен запирать настройку, которую он когда-то менял.
 * Поэтому `updated_by` необязателен: у значения по умолчанию автора нет вовсе.
 */
@Entity('settings')
export class SettingRow {
  ...
  // Время ставит база: «когда это поменяли» сравнивают между строками, а
  // значит оно обязано идти из одних часов, а не из часов того процесса,
  // который принял запрос.
  //
  // Дефолт при этом срабатывает ТОЛЬКО на вставке. Тот, кто пишет сюда
  // upsert, обязан в ветке обновления выставить `updated_at = now()`
  // выражением — не `new Date()`, как это делает сосед `prefs.service.ts`, и
  // не пропустить колонку вовсе: иначе время замрёт на моменте засева и
  // «когда это поменяли» будет врать тем убедительнее, чем дольше живёт
  // инсталляция.
  @Column({ type: 'timestamptz', name: 'updated_at', default: () => 'now()' })
  updatedAt!: Date;

  /** Кто поменял. `null` — значение по умолчанию, его никто не выставлял. */
  @Column({ type: 'uuid', name: 'updated_by', nullable: true })
  updatedBy!: string | null;
}

/**
 * Запись журнала: кто, что, с чего на что и когда.
 *
 * Ник автора лежит снимком рядом с его отпечатком — как в цитатах и
 * упоминаниях: человек переименуется, а запись «Ким закрыл регистрацию»
 * обязана остаться читаемой. Отпечаток отвечает на «кто это был», ник — на
 * «как его тогда звали».
 *
 * `detail` — jsonb, потому что у разных действий разный состав («было/стало» у
 * настройки, причина у бана), и раскладывать это по колонкам значило бы
 * заводить колонку под каждое будущее действие. Значения секретов сюда не
 * пишутся никогда: журнал читает владелец, но читает он его с экрана, который
 * легко показать соседу.
 */
@Entity('audit')
// Журнал читается страницами от свежих к старым, и ключ полный (`at`, `id`) по
// той же причине, что у ленты: в одну миллисекунду попадает несколько записей,
// и страница бы их теряла. DESC живёт только в миграции: декоратор порядка
// сортировки не выражает, а Postgres читает этот индекс и в обратную сторону.
@Index('audit_at_idx', ['at', 'id'])
// «Что делал вот этот» — второй вопрос к журналу и единственный, ради которого
// он фильтруется.
...
export class AuditRow {
  ...
  /** Отпечаток личности автора. `null` — сделала система, а не человек. */
  @Column({ type: 'uuid', nullable: true })
  actor!: string | null;

  @Column({ type: 'text', name: 'actor_nick' })
  actorNick!: string;

  @Column({ type: 'text' })
  action!: string;

  /** На кого/на что подействовали: ключ настройки, отпечаток, слаг канала. */
  @Column({ type: 'text', nullable: true })
  target!: string | null;

  // Дефолт объектом, а не выражением, — как у `reactions`: у jsonb TypeORM
  // сверяет дефолты глубоким сравнением значений, и `() => "'{}'::jsonb"` он
  // счёл бы расхождением с прочитанным из базы.
  @Column({ type: 'jsonb', default: {} })
  detail!: Record<string, unknown>;
}

/** Всё, что знает DataSource. Порядок — как в файле: реестр, чат, личности, инсталляция. */
