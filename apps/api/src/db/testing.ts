 ...
 *
 * Условие у него одно, и оно не выражается в коде: с базой в этот момент никто
 * больше не разговаривает. TRUNCATE берёт исключительную блокировку на каждую
 * названную таблицу, а чужая вставка в этот момент держит одну из них и просит
 * вторую — Postgres разнимает их не очередью, а взаимной блокировкой (40P01), и
 * падает при этом тест, пришедший следом, а не тот, что оставил хвост. Кто
 * обеспечивает тишину — `stopGateway` в gateway/gateway.testkit.ts.
 */
export async function resetDatabase(db: DataSource): Promise<void> {
  await db.query(
    'TRUNCATE servers, channels, attachments, messages, identities, devices, owner_claims, roles, reads, prefs, conversations, pins, settings, audit CASCADE',
  );
}
