...
export interface IdenticonOptions {
  /**
   * Лицо дышит: поле медленно поворачивается и наплывает.
   *
   * По умолчанию — НЕТ, и это не про вкус. Дрейф крутит и масштабирует группу,
   * лежащую под гауссовым размытием, поэтому браузер пересчитывает размытие на
   * каждом кадре, отдельно для каждого лица. Живой замер: 30 лиц в разделе ЛС —
   * 41,8 кадра в секунду, и ровно 60, если выключить дрейф.
   *
   * Просить движение стоит там, где лицо на экране одно и оно — предмет этого
   * экрана: вход, своя карточка личности, окна про одного человека. В списках,
   * стеках, составе и плитках лицо неподвижно, а движение остаётся значить
   * речь — её рисует `speaking`, и оно там всегда к месту.
   */
  alive?: boolean;
}
...
export function identiconSvg(
  ...
  // Дрейф — по запросу, а не по умолчанию. Он анимирует поворот и масштаб
  // группы, лежащей под гауссовым размытием, а это заставляет браузер считать
  // размытие заново на каждом кадре — для каждого лица на экране. Замер у
  // человека с 30 лицами в разделе ЛС: 41,8 кадра в секунду, и ровно 60, если
  // выключить либо дрейф, либо размытие. Поэтому дышит только то лицо, ради
  // которого экран и открыт; в списках, стеках и плитках лица неподвижны.
  if (opts.alive) {
    const drift =
...
export function identicon(fingerprint: string, size: number, opts: IdenticonOptions = {}): string {
  const key = `${fingerprint}|${size}|${opts.alive ? 'a' : ''}`;
