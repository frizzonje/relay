...
export interface TransportHost {
   ...
   * Переподключает дирижёр — к тому же медиасерверу, с новым пропуском: канал
   * в режиме sfu звонит только через него, в p2p никто не уезжает. Mesh это не
   * зовёт: у него своя лестница кончается снятием конкретного пира, а не
   * звонка целиком.
   *
  ...
  /**
   * Транспорт встал: соединения стоят, своё медиа уехало. Только по этой вехе
   * дирижёр и считает медиасервер вернувшимся — выданный пропуск ещё не связь.
   * Mesh это не зовёт: у него нет «встал» на весь звонок, только по пирам.
   */
  transportUp(): void;
  /**
...
export interface VoiceTransport {
  ...
  reset(): void;
  /**
   * Сокет вернулся с ТЕМ ЖЕ id (socket.io восстановил сессию): пересобирать
   * звонок не надо, но пока сигналинг лежал, соединения могли развалиться, а
   * починить их без него было нельзя. Транспорт догоняет упущенное.
   */
  resync?(): void;
}
