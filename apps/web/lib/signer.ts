 ...
 *
 * Интерфейс здесь ровно потому, что реализаций будет две. В браузере ключ —
 ...
 * подписать через мост (следующая задача слоя 2).
 *
...
export type SignerFailure =
  ...
  | 'no-storage'
  /** Оболочка десктопа: веб-путь здесь запрещён, ключ обязан быть нативным. */
  | 'shell'
...
export async function getSigner(opts: { store?: KeyStore; shell?: boolean } = {}): Promise<Signer> {
  if (opts.shell ?? inShell())
    throw new SignerError('shell', 'в оболочке ключ живёт в Rust, а не в webview');
