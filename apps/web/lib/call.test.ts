...
// Тост берём подделкой, а перевод — тождеством: КАКИМИ словами говорят,
// отвечают словари (полноту и разночтения сторожит `lib/i18n/messages.test.ts`),
// а здесь вопрос другой — сказано ли вообще и той ли причиной.
...
describe('конец разговора', () => {
  ...
  it('отказ во входе в комнату беседы кончает звонок у себя — и говорит, почему', async () => {
    const { call, voice } = await talking();
    ...
    expect(voice.leaveVoice).toHaveBeenCalledWith(true);
    expect(call.callRoom()).toBeNull();
    // И человеку об этом говорят. Экран дозвона к этому моменту уже погашен
    // событием `accepted`, а `call-over` не доедет никогда: промолчи мы —
    // человек нажал «Принять», минуту держал системный запрос, разрешил, и
    // сцена голоса просто закрылась бы со статусом «отключено». Молчащий отказ
    // клиент не отличает от удавшегося.
    expect(toast).toHaveBeenCalledWith('refused.voice.not-in-call');
  });

  it('...а вкладке ВНЕ разговора этот отказ сказать нечего', async () => {
    const { call } = await fresh();
    // Шальное событие в обычном голосовом канале: своего разговора у вкладки
    // нет, кончать нечего — и «разговор не собрался» здесь было бы враньём про
    // то, чего человек не делал.
    handlers['voice-refused']({ reason: 'not-in-call' } as never);
