 ...
 * блюра, та же колонка вкладок слева. Разница ровно одна и она по существу: там
 * человек настраивает свою вкладку, здесь — всю инсталляцию, и каждое поле
 * рисуется по КАТАЛОГУ, приехавшему с сервера.
 *
 * Из этой разницы растёт и вторая, видимая: вкладок здесь семнадцать против
 * четырёх, и на телефоне они переносятся по строкам, а не едут лентой вбок.
 * Лента вмещает пять и прячет остальные за краем, о котором пальцу неоткуда
 * узнать.
 *
...
export function AdminDialog() {
  ...
  // Вкладок семнадцать, и в колонку высотой 600 они помещаются не всегда:
  // подтягиваем выбранную в поле зрения. На телефоне подтягивать нечего —
  // вкладки там переносятся по строкам и видны все сразу, — и вызов честно
  // ничего не делает: `block: 'nearest'` на том, что уже в кадре, не двигает
  // ничего. Тот же приём, что в личных настройках.
      ...
      >
        ...
        <nav
          ...
          className={cn(
            ...
            // Мобайл: вкладки переносятся по строкам и видны все сразу. Лента с
            // горизонтальной прокруткой, которая тут была, врала краем: на 375
            // в строку влезает пять вкладок из семнадцати, а прокрутки пальцем
            // не видно — «Обслуживание» просто не существовало для того, кто не
            // догадался потянуть ленту вбок.
            'max-md:w-full max-md:flex-row max-md:flex-wrap max-md:gap-1 max-md:overflow-visible',
            ...
            <Fragment key={item}>
              ...
              <button
                ...
                className={cn(
                  ...
                  // Название не разрывается посреди слова, но и не тянет строку
                  // за край: перенос идёт целыми вкладками. На телефоне поле
                  // вокруг названия чуть уже — иначе семнадцать вкладок заняли
                  // бы пол-экрана прежде, чем показать первое поле.
                  'max-md:shrink-0 max-md:whitespace-nowrap max-md:px-2.5 max-md:py-1.5 max-md:text-[13px]',
                  item === active
                    ? 'text-text-header'
