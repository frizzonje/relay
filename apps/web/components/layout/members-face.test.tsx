 ...
 *
 * Заодно тут закреплено, что молчащее лицо в составе неподвижно: движение в
 * списке лиц стоит кадров всему приложению, и единственное движение, которое
 * лицу здесь положено, — речь.
 */
...
describe('лицо в составе голосового канала', () => {
  ...
  it('молчащий не движется вовсе — ни дрейфа, ни поясов', () => {
    // Состав — это список, а в списке лицо неподвижно: дрейф пересчитывает
    // гауссово размытие каждый кадр, отдельно для каждого лица (см.
    // lib/identicon). В канале на два десятка человек это две дюжины размытий
    // по 60 раз в секунду за то, чего никто не просил. Речь — другое дело: она
    // что-то значит и кончается вместе со словами.
    useVoiceStore.setState({ tiles: [tile({})] });
    const out = markup();
    expect(out).not.toContain('rlDrift');
