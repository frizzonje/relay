...
export interface VoiceTile {
   ...
   * старте/остановке показа, у собеседников приходит с `media-update`. Такую
   * плитку не зеркалим и показываем кадр целиком (object-contain): обрезать
   * чужой рабочий стол по краям нельзя.
   */
  screen: boolean;
  /**
  ...
  net?: TileNet;
  /**
   * Отпечаток ключа владельца плитки — его лицо (см. lib/identicon). Приезжает
   * с presence и потому может опоздать: плитку заводит транспорт, а роли и
   * отпечатки — сигналинг. У гостя по инвайту отпечатка нет вовсе: ключа ему
   * не выдавали, и лица у него не будет — только имя.
   */
  fingerprint?: string;
