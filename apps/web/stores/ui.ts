...
interface UiState {
  ...
  dmRoom: string | null;
  /**
   * Раздел ЛС раскрыт. На десктопе это своя колонка справа от состава: она
   * раздвигает раскладку, а не накрывает соседа, и свёрнутая не занимает
   * ничего — раскладка становится ровно прежней (см. DmDrawer); на телефоне —
   ...
   */
  dmSection: boolean;
  /** Раздел ЛС: выехать панелью справа или уехать за рейку, не оставив следа. */
  toggleDmSection: () => void;
  /**
   * Показать список переписок. Не «переключить»: зовут отсюда те, у кого ответ
   * один, — шаг назад из беседы на телефоне. Тумблер на их месте закрывал бы
   * список ровно тогда, когда его просили открыть.
   */
  showDmList: () => void;
  /**
