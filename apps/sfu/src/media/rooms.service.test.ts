...
describe('комната и роутер', () => {
  ...
  it('двое вошли в пустую комнату одновременно — роутер у них всё равно один', async () => {
    // Так входит вся комната после рестарта медиасервера: круги ожидания у
    // всех заведены одним обрывом и срабатывают в одну секунду. Два роутера на
    // одну комнату — это двое «в канале», которые друг друга не слышат вовсе
    // (consume чужого роутера сервер отвергает): e2e, 24.09.
    const [first, second] = await Promise.all([join('эфир', 'a'), join('эфир', 'b')]);
    expect(second.router).toBe(first.router);
    expect(workers.routers).toHaveLength(1);
    expect(
      rooms
