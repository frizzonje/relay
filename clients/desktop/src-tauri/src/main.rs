    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            *handle.state::<AppState>().settings.lock().unwrap() = loaded;
            ...
            // Выбор источника демонстрации рисует сам движок (на Windows —
            // модальное окно WebView2 поверх нашего). web-UI отбивает нам его
            // открытие и закрытие (payload — bool). Нужно это за двумя вещами:
            //
            // 1) Диагностика. Есть жалоба «нажал демонстрацию, нажал Отмену —
            //    окно встало намертво». По логу сразу видно, дожил ли web-слой до
            //    закрытия пикера: пришло `open` без `close` — встало внутри
            //    модалки движка, пришли оба — дальше смотреть уже наш код.
            // 2) Побудка. Пока модальное окно движка живо, оно крутит СВОЙ цикл
            //    сообщений; после его закрытия наш цикл иногда остаётся без
            //    событий и не перерисовывает окно — снаружи это и выглядит как
            //    «зависло». Дело поправимое: любое сообщение, доставленное в
            //    главный поток, будит цикл, а заодно возвращает окну фокус.
            let h = handle.clone();
            handle.listen("screen-picker", move |event| {
                let open = serde_json::from_str::<bool>(event.payload()).unwrap_or(false);
                ulog(&format!(
                    "screen picker {}",
